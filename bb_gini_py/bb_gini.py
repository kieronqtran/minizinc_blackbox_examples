...
if __name__ == "__main__":
  ...
  for line in stdin:
    ...
    parts = line.split('E')
    assert len(parts) == 3, f"Expected 2 occurences of E in the input, but split into {parts}"
    ...
    print(f"{' '.join([str(i) for i in int_out])} E {' '.join([str(f) for f in float_out])} E", flush=True)
