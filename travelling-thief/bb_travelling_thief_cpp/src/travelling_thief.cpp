...
extern "C" {
  ...
  void fzn_blackbox(const int* int_in, size_t int_in_len, const double *float_in,
    ...
    double nu = (max_speed - min_speed) / (knapsack_capacity / 1000);
    int order_len = int_in[4];
    int distances_n_rows = int_in[5];
    int distances_n_cols = int_in[6];
    int weights_len = int_in[7];
    std::vector<int> orders(int_in + 8, int_in + 8 + order_len);
    std::map<std::pair<int,int>, int> city_distances;
    for (int i = 0; i < distances_n_rows; ++i) {
      for (int j = 0; j < distances_n_cols; ++j) {
        city_distances[std::make_pair(i, j)] = int_in[8 + order_len + i * distances_n_cols + j];
      }
    }
    std::vector<int> weights(int_in + 8 + order_len + distances_n_rows * distances_n_cols,
                             int_in + 8 + order_len + distances_n_rows * distances_n_cols + weights_len);
